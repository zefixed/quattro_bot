...
COPY public.cards (id, client_id, card_number, expiration_date, status, balance) FROM stdin;
\.
...
COPY public.clients (id, first_name, last_name, patronymic, email, telegram_id) FROM stdin;
\.
...
COPY public.loans (id, client_id, amount, interest_rate, status, due_date) FROM stdin;
\.
...
COPY public.transactions (id, client_id, amount, transaction_type, recipient_id) FROM stdin;
\.
